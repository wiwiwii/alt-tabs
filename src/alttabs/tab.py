from alttabs.instrument import FretBoard, PlayedNote


class TabParseError(Exception):
    pass


class TabStringLine:
    """
    One ASCII tab line for one string.

    Example content after label stripping:
    |--3--10--0--|
    """

    def __init__(self, string: int, content: str, raw_label: str | None = None):
        self.string = string
        self.content = content
        self.raw_label = raw_label


class TabEvent:
    """
    Notes starting at one source column.

    at_column:
        Character index in the normalized content area.
    notes:
        Played notes starting at that column.
    """

    def __init__(self, at_column: int, notes: list[PlayedNote]):
        self.at_column = at_column
        self.notes = notes


class ParsedTab:
    """
    Full parsed tab block.

    string_lines:
        Ordered lines as given in the input.
    events:
        Musical events grouped by onset column.
    width:
        Max normalized content width.
    """

    def __init__(
        self, string_lines: list[TabStringLine], events: list[TabEvent], width: int
    ):
        self.string_lines = string_lines
        self.events = events
        self.width = width


class TabParser:
    """
    Narrow ASCII tab parser.

    Supported:
    - lines like e|--0--3--|
    - lines like |--0--3--|
    - multi-digit frets
    - chords by same-column onset

    Not supported yet:
    - h / p / / / \\ / b / r / x / ~ / parentheses
    - rhythmic inference
    """

    NOTE_LABEL_TO_STRING = {
        "e": 1,
        "B": 2,
        "G": 3,
        "D": 4,
        "A": 5,
        "E": 6,
        "g": 1,
        "d": 2,
        "a": 3,
        "E_bass": 4,
    }

    def __init__(self, fretboard: FretBoard):
        self.fretboard = fretboard

    def parse(self, text: str) -> ParsedTab:
        raw_lines = [line.rstrip() for line in text.splitlines() if line.strip()]
        if not raw_lines:
            raise TabParseError("Empty tab")

        string_lines = self._parse_string_lines(raw_lines)
        width = max(len(line.content) for line in string_lines)
        normalized_lines = self._pad_lines(string_lines, width)
        events = self._extract_events(normalized_lines)

        return ParsedTab(normalized_lines, events, width)

    def _parse_string_lines(self, raw_lines: list[str]) -> list[TabStringLine]:
        parsed = []

        expected_strings = [spec.number for spec in self.fretboard.tuning.strings]

        if len(raw_lines) != len(expected_strings):
            raise TabParseError(
                f"Expected {len(expected_strings)} lines for this instrument, got {len(raw_lines)}"
            )

        for idx, raw in enumerate(raw_lines):
            string_number, label, content = self._split_label_and_content(
                raw, fallback_index=idx
            )
            parsed.append(
                TabStringLine(string=string_number, content=content, raw_label=label)
            )

        parsed_numbers = [line.string for line in parsed]
        if sorted(parsed_numbers) != sorted(expected_strings):
            raise TabParseError(
                f"Parsed string numbers {parsed_numbers} do not match instrument strings {expected_strings}"
            )

        return parsed

    def _split_label_and_content(self, raw: str, fallback_index: int):
        """
        Accept:
        e|--0--|
        B|--1--|
        |--0--|

        If there is no label, map by physical line order to the fretboard's tuning order.
        """
        if "|" not in raw:
            raise TabParseError(f"Missing '|' separator in line: {raw}")

        before_bar, after_bar = raw.split("|", 1)
        label = before_bar.strip()

        if label:
            string_number = self._string_number_from_label(label, raw, fallback_index)
            return string_number, label, "|" + after_bar

        tuning_order = [spec.number for spec in self.fretboard.tuning.strings]
        string_number = tuning_order[fallback_index]
        return string_number, None, "|" + after_bar

    def _string_number_from_label(
        self, label: str, raw: str, fallback_index: int
    ) -> int:
        """
        Guitar:
            e B G D A E
        Bass:
            G D A E

        Ambiguity: uppercase E appears on guitar low E and bass low E.
        Resolve using line count / tuning membership / line order.
        """
        tuning_numbers = [spec.number for spec in self.fretboard.tuning.strings]

        if label == "e":
            if 1 in tuning_numbers:
                return 1
            raise TabParseError(f"Label '{label}' not valid for this instrument: {raw}")

        if label in ("B", "G", "D", "A"):
            mapping = {"B": 2, "G": 3, "D": 4, "A": 5}
            string_number = mapping[label]
            if string_number not in tuning_numbers:
                raise TabParseError(
                    f"Label '{label}' not valid for this instrument: {raw}"
                )
            return string_number

        if label == "E":
            if len(tuning_numbers) == 6:
                return 6
            if len(tuning_numbers) == 4:
                return 4

        raise TabParseError(
            f"Unsupported or ambiguous string label '{label}' in line: {raw}"
        )

    def _pad_lines(
        self, string_lines: list[TabStringLine], width: int
    ) -> list[TabStringLine]:
        result = []
        for line in string_lines:
            padded = line.content.ljust(width, "-")
            result.append(TabStringLine(line.string, padded, line.raw_label))
        return result

    def _extract_events(self, string_lines: list[TabStringLine]) -> list[TabEvent]:
        """
        Scan all lines. Whenever a digit starts at column c on a string, parse the full
        integer fret number on that line, then create/group an event at column c.
        """
        events_by_column: dict[int, list[PlayedNote]] = {}

        for line in string_lines:
            col = 0
            content = line.content

            while col < len(content):
                ch = content[col]

                if ch.isdigit():
                    if col > 0 and content[col - 1].isdigit():
                        col += 1
                        continue

                    start = col
                    end = col
                    while end < len(content) and content[end].isdigit():
                        end += 1

                    fret_text = content[start:end]
                    fret = int(fret_text)

                    played = self.fretboard.note_at(line.string, fret)

                    if start not in events_by_column:
                        events_by_column[start] = []
                    events_by_column[start].append(played)

                    col = end
                    continue

                col += 1

        return [
            TabEvent(at_column=column, notes=events_by_column[column])
            for column in sorted(events_by_column.keys())
        ]
