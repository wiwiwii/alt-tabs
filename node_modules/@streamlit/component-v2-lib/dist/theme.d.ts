 ...
 */
type CamelToKebab<S extends string> = S extends `${infer T}${infer U}` ? `${T extends Capitalize<T> ? "-" : ""}${Lowercase<T>}${CamelToKebab<U>}` : S;
/**
 * Utility type to convert an object type to CSS custom properties
 * with a specified prefix and kebab-case naming
 */
type ObjectToCssCustomProperties<T extends object, Prefix extends string = "--st"> = {
 ...
 * StreamlitTheme.
 */
export type StreamlitThemeCssProperties = ObjectToCssCustomProperties<StreamlitTheme>;
export {};
