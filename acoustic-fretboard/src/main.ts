...
let selected: Cell = { stringNumber: 6, fret: 3 };
...
app.innerHTML = `
  <main class="shell">
    <section class="panel">
      <div class="topbar">
        <div>
          <div class="eyebrow">Acoustic guitar prototype</div>
          <h1>Martin-inspired fretboard selector</h1>
        </div>
      ...
      <div class="legend">
        ...
        <span>Theme: ebony board · pearl dots · ivory nut</span>
      </div>
    </section>
  </main>
`;
...
function yOnString(stringNumber: StringNumber, x: number): number {
  ...
  const fraction = (stringNumber - 1) / 5;
...
function openCellPolygon(stringNumber: StringNumber): string {
  ...
  const bottomY =
    stringNumber === 6
      ? yOnString(6, geometry.boardLeft) + 12
      : midpointY(
          stringNumber,
          (stringNumber + 1) as StringNumber,
          geometry.boardLeft,
...
function inlayMarkup(): string {
  ...
  for (const fret of single) {
    ...
    circles.push(
      `<circle cx="${x}" cy="${geometry.centerY}" r="7" class="inlay" />`,
    );
  }
  ...
  for (const fret of double) {
    const x =
      (geometry.fretBoundaries[fret - 1] + geometry.fretBoundaries[fret]) / 2;
    circles.push(
      `<circle cx="${x}" cy="${geometry.centerY - 22}" r="6.5" class="inlay" />`,
    );
    circles.push(
      `<circle cx="${x}" cy="${geometry.centerY + 22}" r="6.5" class="inlay" />`,
    );
...
function stringLabels(): string {
  ...
  for (let n = 1 as StringNumber; n <= 6; n += 1) {
...
function render(): void {
  ...
  const fretLines = Array.from(
    ...
    (_, idx) => {
      ...
      return `<line x1="${x}" y1="${y1}" x2="${x}" y2="${y2}" class="fret-line ${idx === 0 ? "nut-line" : ""}" />`;
    },
  ).join("\n");

  const stringLines = Array.from({ length: 6 }, (_, idx) => {
    ...
    return `
      <line
        ...
        class="string-line"
        style="stroke-width:${stringGauge(n)}"
      />
    `;
  }).join("\n");
  ...
  svgWrap.innerHTML = `
    <svg
      ...
      class="fretboard"
      role="img"
      aria-label="Acoustic guitar fretboard selector"
    >
      <defs>
